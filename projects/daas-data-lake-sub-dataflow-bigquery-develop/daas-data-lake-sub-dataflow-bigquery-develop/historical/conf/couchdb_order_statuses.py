...
historical_sql = """copy into @json_unload_file_stage/{table}/{subdir}/{table}_json
             from (
                    select OBJECT_INSERT(OBJECT_INSERT(OBJECT_INSERT(OBJECT_INSERT(OBJECT_INSERT(OBJECT_INSERT(OBJECT_INSERT(OBJECT_INSERT(OBJECT_INSERT(KEYS_TO_SNAKE(REMOVE_NULLS(doc)), 
                                  'status_from',doc:changes[0][1][1][0]:status),
                                  'status_to', DOC:changes[0][1][1][1]:status),  
                                  'changes', doc:changes[0][0], true),
                    ...
                    from tom_changes_feed
                    where DOC:model = 'o' and DOC:change_fields[0] = 'status'
