...
historical_sql = """copy into @json_unload_file_stage/{table}/{subdir}/{table}_json
             from (
                    select OBJECT_INSERT(OBJECT_INSERT(OBJECT_INSERT(OBJECT_INSERT(OBJECT_INSERT(OBJECT_INSERT(OBJECT_INSERT(OBJECT_INSERT(KEYS_TO_SNAKE(REMOVE_NULLS(doc)),
                                  'picker_id', changes.value[1][1][1]:picker_id),  
                                  'changes', changes.value[0], true), 
                    ...
                    from tom_changes_feed, lateral flatten(doc:changes) as changes
                    where DOC:model = 'o' and changes.value[0] = 'picker_id'
