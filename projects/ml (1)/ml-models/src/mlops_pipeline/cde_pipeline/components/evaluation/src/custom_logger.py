...
def set_level(level, logger, internal_logging) -> None:
    """
    function to set level of log
    """

    level_dict = {"INFO": internal_logging.INFO,
                  "ERROR": internal_logging.ERROR}
...
def set_log(
    level=None, logger=None, message: str = None, base_log_params: dict = None
) -> None:
    """
    function to set the log
    """

    log_level_dict = {"INFO": logger.info, "ERROR": logger.error}
...
def ml_logger(
    type_log: str = "INFO",
    component: str = "NOTSET1",
    message: str = "NOTSET",  # info
    status_code: str = "empty",
    json_response: str = "",
    traceback: str = "",
) -> None:
    """
    ...
    set_level(level=type_log, logger=logger, internal_logging=logging)

    if type_log == "ERROR":
        log_format = (
            '{"component":"%(component)s", "message":"%(message)s", \
            "status_code":"%(status_code)s",'
            '"json_response":"%(json_response)s", "traceback":"%(traceback)s"}'
        )
