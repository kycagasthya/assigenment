...
def convert_label_map_to_categories(label_map,
    ...
    Args:
      label_map: a StringIntLabelMapProto or None.  If None, a default
        categorieslist is created with max_num_classes categories.
      max_num_classes: maximum number of (consecutive) label indices to include.
      use_display_name: (boolean) choose whether to load 'display_name' field as
        category name.  If False or if the display_name field does not exist,
    ...
    for item in label_map.item:
        ...
        if item.id not in list_of_ids_already_added:
            ...
            if item.HasField('frequency'):
                if item.frequency == string_int_label_map_pb2.LVISFrequency.\
                        Value('FREQUENT'):
                    category['frequency'] = 'f'
                elif item.frequency == string_int_label_map_pb2.LVISFrequency.\
                        Value('COMMON'):
...
def get_label_map_dict(label_map_path_or_proto,
    ...
    Args:
      label_map_path_or_proto: path to StringIntLabelMap proto text file or the
        proto itself.
      use_display_name: whether to use the label map items' display names as
      keys.
    ...
    Raises:
      ValueError: if fill_in_gaps_and_background and label_map has non-integer
      or negative values.
    """
...
def get_label_map_hierarchy_lut(label_map_path_or_proto,
    ...
    The function returns the ancestors and descendants as separate look up
     tables (LUT) numpy arrays of shape [max_id, max_id] where lut[i,j] = 1
     when there is a hierarchical relationship between class i and j.
...
def create_categories_from_labelmap(label_map_path, use_display_name=True):
    ...
    which  has the following keys:
      ...
      'keypoints': a dictionary of keypoint string label to integer id. It is
       only returned when available in label map proto.
