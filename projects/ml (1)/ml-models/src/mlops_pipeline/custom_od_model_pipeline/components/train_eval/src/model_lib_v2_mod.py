...
def _compute_losses_and_predictions_dicts(
    ...
    Args:
      ...
      labels: A dictionary of groundtruth tensors post-unstacking. The original
        ...
        `inputs.eval_input`. The shapes may have been modified by unstacking
        with
        `model_lib.unstack_batch`. However, the dictionary includes the
        following
        fields.
          ...
          labels[fields.InputDataFields.groundtruth_group_of] is a tf.bool
          tensor
    ...
    if add_regularization_loss:
        # As we figure out mixed precision & bfloat 16, we may
      ...
      labels: A dictionary of groundtruth tensors. This method unstacks
        ...
        the form returned by `inputs.train_input` and `inputs.eval_input`.
          ...
          labels[fields.InputDataFields.groundtruth_weights] is a
            [batch_size, num_boxes] float32 tensor containing groundtruth
            weights
            for the boxes.
          -- Optional --
          labels[fields.InputDataFields.groundtruth_instance_masks] is a
          ...
          labels[fields.InputDataFields.groundtruth_dp_surface_coords] is a
            [batch_size, num_boxes, max_sampled_points, 4] float32 tensor with
            the
            surface coordinates for each point. Each surface coordinate is of
            the
...
def is_object_based_checkpoint(checkpoint_path):
    """Returns true if `checkpoint_path` points to
    an object-based checkpoint."""
...
def load_fine_tune_checkpoint(model, checkpoint_path, checkpoint_type,
    ...
    Args:
      ...
      run_model_on_dummy_input: Whether to run the model on a dummy input in
        order
...
def get_filepath(strategy, filepath):
    ...
    if strategy.extended.should_checkpoint:
        ...
        # Replace with the public API when TF exposes it.
...
def train_loop(
    ...
    Args:
      ...
      checkpoint_max_to_keep:
        int, the number of most recent checkpoints to keep in the model
        directory.
        ...
        with strategy.scope():
            with tf.compat.v2.summary.record_if(
                ...
                def _sample_and_train(strategy, train_step_fn, data_iterator):
                    ...
                    return reduce_dict(
                        strategy, per_replica_losses_dict,
                ...
                for _ in range(global_step.value(), train_steps,
                    ...
                    if global_step.value() - logged_step >= 100:
                        ...
                        tf.logging.info(
                            pprint.pformat(logged_dict_np, width=40))
                        logged_step = global_step.value()
...
def prepare_eval_dict(detections, groundtruth, features):
      ...
      groundtruth: `inputs.eval_input` returns an eval dataset of (features,
        ...
        Please note that:
          ...
          * fields.InputDataFields.groundtruth_labeled_classes must be
            0-indexed and in its multi-hot repesentation.
      features: `inputs.eval_input` returns an eval dataset of (features,
      labels)
        tuple. This argument must be set to a dictionary containing the
        following
    ...
    if fields.InputDataFields.groundtruth_labeled_classes in groundtruth:
        groundtruth[
            fields.InputDataFields.groundtruth_labeled_classes] = tf.pad(
    ...
    if use_original_images:
        ...
        true_image_shapes = features[fields.InputDataFields.true_image_shape][:,
                            :3]
...
def eager_eval_loop(
    ...
    Args:
      ...
      global_step: A variable containing the training step this model was
        trainedto. Used for logging purposes.
    ...
    row_dict = {}
    row_dict['step'] = global_step.numpy()
    field_names = ['step'] + list(eval_metrics.keys())
    global header_written
    if not header_written:
        with open('metrics.csv', 'w') as csv_fl:
            dict_obj = DictWriter(csv_fl, fieldnames=field_names)
            dict_obj.writeheader()
            header_written = True
    for k, v in eval_metrics.items():
        if tf.is_tensor(v):
            row_dict[k] = round(v.numpy(), 5)
        else:
            row_dict[k] = round(v, 5)

    with open('metrics.csv', 'a') as csv_fl:
        dict_obj = DictWriter(csv_fl, fieldnames=field_names)
        dict_obj.writerow(row_dict)
...
def eval_continuously(
    ...
    Args:
      ...
      sample_1_of_n_eval_examples: Integer representing how often an eval
        example should be sampled. If 1, will sample all examples.
      sample_1_of_n_eval_on_train_examples: Similar to
        `sample_1_of_n_eval_examples`, except controls the sampling of training
        data for evaluation.
      use_tpu: Boolean, whether training and evaluation should run on TPU.
      override_eval_num_epochs: Whether to overwrite the number of epochs to 1
        for eval_input.
    ...
    tf.compat.v2.print('In eval function')
    get_configs_from_pipeline_file = MODEL_BUILD_UTIL_MAP[
        'get_configs_from_pipeline_file']
    create_pipeline_proto_from_configs = MODEL_BUILD_UTIL_MAP[
        'create_pipeline_proto_from_configs']
    ...
    tf.compat.v2.print('Checkpoints found: ' + str(
        tf.train.get_checkpoint_state(
            checkpoint_dir).all_model_checkpoint_paths))
    for latest_checkpoint in tf.compat.v2.train.get_checkpoint_state(
            checkpoint_dir).all_model_checkpoint_paths:
        tf.compat.v2.print('Working on ckpt: ' + str(latest_checkpoint))
        ckpt = tf.compat.v2.train.Checkpoint(
            step=global_step, model=detection_model, optimizer=optimizer)

        # We run the detection_model on dummy inputs in order to ensure that the
        # model and all its variables have been properly constructed.
        # Specifically,
        # this is currently necessary prior to (potentially)
        ...
        tf.compat.v2.print('Evaluation on step %d', global_step.numpy())
        with summary_writer.as_default():
            eager_eval_loop(
                detection_model,
                configs,
    ...
    tf.compat.v2.print('Exiting evaluation at step %d', global_step.numpy())
