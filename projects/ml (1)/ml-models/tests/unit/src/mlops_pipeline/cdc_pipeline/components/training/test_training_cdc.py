...
"""This module has unit test for training_cdc"""

import sys
import unittest
from unittest.mock import patch
sys.path.insert(0, 'src/mlops_pipeline/cdc_pipeline/components/training/src')
from training_cdc import get_auth_token, uptraining, training_from_scratch,   \
                     get_latest_version, get_default_version, training,    \
                     create_version_name, lro, cdc_train
...
class TestTraining(unittest.TestCase):
    ...
    @patch('training_cdc.google.auth.transport.requests.Request')
    @patch('training_cdc.google.auth.default')
    @patch('training_cdc.requests.post')
    ...
    @patch('training_cdc.google.auth.transport.requests.Request')
    @patch('training_cdc.google.auth.default')
    @patch('training_cdc.requests.get')
...
processors/4fd83649b68e0f12/processorVersions/16dcb57b63f2ef30'}}}
    ...
    @patch('training_cdc.create_version_name')
    @patch('training_cdc.training')
    @patch('training_cdc.lro')
    def test_cdc_train_success(self, mock_lro, mock_training,
...
processors/4fd83649b68e0f12/processorVersions/ea4f387463d05d0'},
        ...
        cdc_train(self.project_number, self.processor_id, self.version_id,
                  self.location, self.check_status_after, VersionName())


    @patch('training_cdc.create_version_name')
    @patch('training_cdc.training')
    def test_cdc_train_error(self, mock_training, mock_create_version_name):
        mock_create_version_name.return_value = 'v29'
