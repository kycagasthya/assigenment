...
import os
#import gc
def get_overlay_objects(slide_name,
                        all_csv_paths, all_slide_paths, output_dir,
                        label_dir=\
                        "/home/jupyter/seattle-quantiphi/\
                        viz_tutorial/csvs/gt_labels",
                        plot_size=1920,
                        return_label_object=True):
    print("plot_size "+ plot_size)
    if not slide_name.endswith(".csv"):
        slide_name+=".csv"
    pred_csv_path = [path for path in all_csv_paths if slide_name in path][0]
    pred_df = pd.read_csv(pred_csv_path)
    pred_df["preds"] = pred_df["preds"].apply(lambda x:
                                              x.lower().replace(" ", "_"))
    pred_df["preds"] = pred_df["preds"].apply(
        lambda x: x.replace("necrotic", "exclusion"))
    path_to_slide_on_machine = find_local_path(
        pred_csv_path[:-4].rsplit("/", 1)[1], all_slide_paths)
    ...
    if return_label_object and os.path.isfile(label_csv_path):
        ...
        label_df["preds"] = label_df["preds"].apply(lambda x:
                                                    x.lower().replace(" ", "_"))
        label_df["preds"] = label_df["preds"].apply(
            lambda x: x.replace("necrotic", "exclusion"))
        label_df.drop(columns=["slide_path"], inplace=True)
        label_overlay = OverlayTool(
            label_df, output_dir, path_to_slide_on_machine)
...
def do_evaluation(all_csv_paths,
    ...
    def do_binary(pred):
        ...
        if pred in (["no_tissue", "out_of_focus"]):
            ...
            return "GOOD"
    temp_dir = tempfile.mkdtemp(prefix="tmp_data_")
    slide_prediction_df = pd.DataFrame(columns=class_list)
    for _, f_name in tqdm(enumerate(all_csv_paths), total=(len(all_csv_paths))):
        edge_preds, _ = get_overlay_objects(
            slide_name=f_name, all_csv_paths=all_csv_paths,
            all_slide_paths=all_slide_paths,
            output_dir=temp_dir, plot_size=100, return_label_object=False)
        ...
        edge_df["folder_gt_label"] = determine_local_label(
            edge_preds.path_to_slide)
        slide_prediction_df = pd.concat([slide_prediction_df, edge_df],)
    slide_prediction_df.fillna(0, inplace=True)
    slide_prediction_df["heuristic_binary_pred"] =\
    slide_prediction_df.heuristic_pred.apply(do_binary)
    slide_prediction_df["folder_binary_gt_label"] =\
    slide_prediction_df.folder_gt_label.apply(do_binary)
    return slide_prediction_df
def return_path_dirs(
    path_to_oof_dir = "/home/jupyter/seattle-quantiphi/data/Focus/Out_Focus",
    path_to_no_tissue_dir = "/home/jupyter/seattle-quantiphi/\
    data/Tissue/No_Tissue",
    path_to_dim_dir = "/home/jupyter/seattle-quantiphi/data/Stained/Dim",
    path_to_str_dir = "/home/jupyter/seattle-quantiphi/data/Stained/Yes",
    path_to_neg_dir = "/home/jupyter/seattle-quantiphi/data/Stained/No",
    path_to_he_dir = "/home/jupyter/seattle-quantiphi/data/H&E"):
    """ Return local directory paths """
    return path_to_oof_dir, path_to_no_tissue_dir, path_to_dim_dir,\
           path_to_str_dir, path_to_neg_dir, path_to_he_dir
def find_local_path(f_name, all_file_paths):
    ...
    if ".svs" not in f_name and ".qptiff" not in f_name:
        try:
            ...
            return [path for path in all_file_paths if f_name+".qptiff"
                    in path][0]
    else:
...
def get_all_paths(list_of_dir_paths):
    ...
    def get_full_paths(dir_path):
        """ Helper Function """
        return [os.path.join(dir_path, f_name) for
                f_name in os.listdir(dir_path)]
    all_paths = [get_full_paths(dir_path) for
                 dir_path in list_of_dir_paths]
    all_paths = [path for path_list in all_paths for
                 path in path_list]
    return all_paths
...
class OverlayTool:
    """ This class if a bundling of the above functions
    in a custom configuration """
    def __init__(
        self, df, output_dir, path_to_slide, cmap_name=None,
        gt_slide_label=None, uncertain_class="-1",
        ...
        Args:
            ...
            path_to_slide (str): Path to the slide (on local where
            it can be loaded)
            gt_slide_label (str, optional): The ground truth label to apply
            max_size (int, optional): The maximum side length to
            constrain the overlay's longest side to
            cmap_name (str, optional): Name of the seaborn color
            palette to use.
                LINK --> https://seaborn.pydata.org/
                tutorial/color_palettes.html
            uncertain_class (str, optional): The name
            given to the uncertain class
                - Often this is -1
            max_edge_size (int, optional): The maximum
            edge size to use when
                resizing/downsampling the slide.
            original_tile_size (int, optional): Original
            size of the tiles that were
                predicted upon by the model
            class_list (tuple, optional): List of classes to be
            used that will tie the
                label in the df to a specific colour in the
    ...
    def initialize_frac_maps(self):
        """ Function to initialize the fractional maps used in
        heuristic labelling
        Args:
        ...
        tissue_value_counts = self.df[self.df.preds!="no_tissue"]\
        ["preds"].value_counts()
        self.frac_map_tissue = {k:0.0 for k in self.classes if k!="no_tissue"}
        self.frac_map_tissue.update({k:v/tissue_value_counts.sum()
                                     for k,v in tissue_value_counts.items()})
        ...
        self.frac_map_all.update({k:v/max(known_value_counts.sum(),
                                          n_tiles_total)
        ...
        if n_tiles_total>known_value_counts.sum():
            self.frac_map_all["no_tissue"]=(known_value_counts.get(
                "no_tissue",0)+(n_tiles_total-known_value_counts.sum(
            )))/n_tiles_total
    def initialize_color(self):
        ...
        self.cpal_map = sns.color_palette(self.cmap_name,
                                          len(self.classes))
        ...
        self.class_cmap = {c:self.class_cpal[i] for i,c
    ...
    def initialize_dwnsampled_slide(self):
        ...
        self.total_downscale_factor = \
        self.downscale_factor*self.start_dwnsample
        self.new_tile_size = int(np.ceil(self.original_tile_size/(
            self.total_downscale_factor)))
        self.slide_new_dim = (
    ...
    def initialize_lbl_color_map(self):
        ...
        Returns:
            None; The following attributes are set:
                --> self.c_lbl_map
        """
        self.c_lbl_map = np.ones((self.slide_new_dim[1],
                                  self.slide_new_dim[0],
        ...
        for xmin, ymin, label in self.df.to_numpy():
            ...
            if label==self.uncertain_class:
                self.c_lbl_map[ymin:ymin+self.new_tile_size,
                               xmin:xmin+self.new_tile_size, :] = (0,0,0)
            else:
                self.c_lbl_map[ymin:ymin+self.new_tile_size,
                               xmin:xmin+self.new_tile_size, :] = \
                self.class_cmap[label]
    def heuristic_slide_classification(self,
        ...
        Args:
            str_thresh (float, optional): Lower threshold for
            fractional slide level determination
                --> Strong-Positive Label
            oof_thresh (float, optional): Lower threshold for
            fractional slide level determination
                --> Out-Of-Focus Label
            dim_thresh (float, optional): Lower threshold for
            fractional slide level determination
                --> Dim-Positive Label
            neg_thresh (float, optional): Lower threshold for
            fractional slide level determination
                --> Negative Label
            he_thresh (float, optional): Lower threshold for
            fractional slide level determination
                --> H&E Label
            not_thresh (float, optional): Lower threshold for
            fractional slide level determination
                --> No-Tissue Label
            ...
            if self.frac_map_tissue.get("strong_positive", 0.0) <= str_thresh:
                if self.frac_map_tissue.get("out_of_focus", 0.0) <= oof_thresh:
                    if all_positive_tissue <= dim_thresh:
                        if self.frac_map_tissue.get("negative",
                                                    0.0) <= neg_thresh:
                            if self.frac_map_tissue.get("he",
                                                        0.0) <= he_thresh:
    ...
    def plot_merged(self, fig_multiplier=20, bg_opacity=0.65,
                    gt_slide_label=None, subregion=None,
                    save_to_dir=None):
        """ Method to plot color map overlayed ontop of the
        downsampled image
        Args:
        ...
        if subregion is not None:
            merged_img = self.merged_img[subregion[1]:subregion[3],
        ...
        legend_elements.append(Patch(facecolor=(1,1,1,1-bg_opacity),
                                     edgecolor=(0,0,0,0.1), label="background"))
        legend_elements.append(Patch(facecolor=(0,0,0,1-bg_opacity),
                                     label="uncertain"))
        # Craft the Figure to be displayed
        plt.figure(figsize=(fig_multiplier,fig_multiplier*(self.slide_new_dim[
            1]/self.slide_new_dim[0])))
        plt.imshow(merged_img)
        
        # jg - hide label and legend - only show ground truth if available
        ...
        if subregion is not None:
            plt.xticks(ticks=np.linspace(0, subregion[2]-subregion[0],
                                         10), labels=np.linspace(
                subregion[0], subregion[2], 10, dtype=np.int32))
            plt.yticks(ticks=np.linspace(0, subregion[3]-subregion[
                1], 10), labels=np.linspace(subregion[1],
                                            subregion[3], 10, dtype=np.int32))
            title_str+=f"\nSUBREGION: {subregion}"
        plt.title(title_str, fontweight="bold")
        plt.grid(color="gray", linestyle=":")
        # If we want to save to file. Do so
        if save_to_dir is not None:
            plt.savefig(os.path.join(save_to_dir, "color_maps",
                                     self.path_to_slide.rsplit("/", 1
                                                              )[1].split(".")[
                                         0]+".png"), dpi=150)
        # Display the image
        plt.show()
    def plot_slide_only(self, fig_multiplier=20, gt_slide_label=None,
                        subregion=None, save_to_dir=None):
        ...
        # Drill down to a specific subregion of the image if requested
        if subregion is not None:
            dwn_slide = self.dwn_slide[subregion[1]:subregion[3],
                                       subregion[0]:subregion[2], :]
        else:
            dwn_slide = self.dwn_slide
        # Craft the Figure to be displayed
        plt.figure(figsize=(fig_multiplier,fig_multiplier*(
            self.slide_new_dim[1]/self.slide_new_dim[0])))
        ...
        if subregion is not None:
            ...
            plt.xticks(ticks=np.linspace(0, subregion[2]-subregion[0],
                                         10), labels=np.linspace(
                subregion[0], subregion[2], 10, dtype=np.int32))
            plt.yticks(ticks=np.linspace(0,
                                         subregion[3]-subregion[1],
                                         10), labels=np.linspace(subregion[1],
                                                                 subregion[3],
                                                                 10,
                                                                 dtype=\
                                                                 np.int32))
        ...
        # If we want to save to file. Do so
        if save_to_dir is not None:
            plt.savefig(os.path.join(save_to_dir, "slide_only",
                                     self.path_to_slide.rsplit("/", 1)
                                     [1].split(".")[0]+".png"), dpi=150)
        # Display the image
        plt.show()
    def plot_side_by_side(self, bg_opacity=0.65, fig_multiplier=20,
                          gt_slide_label=None, subregion=None):
        """ Method to plot the overlayed color map grid beside
        the original image
        Args:
            bg_opacity (float, optional):
            fig_multiplier (int, optional):
        ...
        legend_elements.append(Patch(facecolor=(1,1,1,1-bg_opacity),
                                     edgecolor=(0,0,0,0.1), label="background"))
        legend_elements.append(Patch(facecolor=(
            0,0,0,1-bg_opacity), label="uncertain"))
        # Craft the Figure to be displayed
        plt.figure(figsize=(fig_multiplier,fig_multiplier*(
            self.slide_new_dim[1]/self.slide_new_dim[0])))
        # COLOR MAP OVERLAY
        ...
        if subregion is not None:
            title_str+=f"\nSUBREGION: {subregion}"
            plt.xticks(ticks=np.linspace(0, subregion[2]-subregion[0], 10),
                       labels=np.linspace(subregion[0], subregion[2],
                                          10, dtype=np.int32))
            plt.yticks(ticks=np.linspace(0,
                                         subregion[3]-subregion[1], 10),
                       labels=np.linspace(subregion[1],
                                          subregion[3], 10, dtype=np.int32))
        plt.title(title_str, fontweight="bold")
        plt.grid(color="gray", linestyle=":")
        # RAW IMAGE DISPLAY
        ...
        if subregion is not None:
            title_str+=f"\nSUBREGION: {subregion}"
            plt.xticks(ticks=np.linspace(0, subregion[2]-subregion[0], 10),
                       labels=np.linspace(subregion[0], subregion[2], 10,
                                          dtype=np.int32))
            plt.yticks(ticks=np.linspace(0, subregion[3]-subregion[1], 10),
                       labels=np.linspace(subregion[1],
                                          subregion[3], 10, dtype=np.int32))
        plt.title(title_str, fontweight="bold")
        plt.grid(color="gray", linestyle=":")
        plt.show()
def mass_visualization(list_of_names, plot_size, all_csv_paths,
                       all_slide_paths, show_frac_map=True,
                       style="side_by_side"):
    """ Tool allowing for visualization of multiple images """
    temp_dir_mass = tempfile.mkdtemp(prefix="tmp_data_mass_")
    for _, f_name in enumerate(list_of_names):
        edge_preds, gt_labels = get_overlay_objects(
            slide_name=f_name, plot_size=plot_size,
            all_csv_paths=all_csv_paths,
            all_slide_paths=all_slide_paths,
            output_dir=temp_dir_mass)
        print("\n\n--------------------------------------------------------\
              ---------------------------------------------------------\n\n")
        ...
        if gt_labels and show_frac_map:
            ...
            for k,v in {k:v for k,v in sorted(gt_labels.frac_map_tissue.items(),
                                              key=lambda x: x[1],
                                              reverse=True)}.items():
                print(f"{k}\n\t%{v*100:0.2f}")
        if show_frac_map:
            print("\n\n---------- PREDICTION DISTRIBUTION ----------\n")
            for k,v in {k:v for k,v in sorted(
                edge_preds.frac_map_tissue.items(),
