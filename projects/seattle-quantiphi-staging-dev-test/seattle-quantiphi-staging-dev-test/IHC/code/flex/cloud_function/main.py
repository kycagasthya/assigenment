...
def hello_gcs(event, context):   
    ...
    template_path = 'gs://seagen_dataflow/templates/dev_ihc/v2.json'
    job_name_prefix = "dev-flex-ihc-v2"
    now = time.strftime("%Y%m%d-%H%M%S")
    jobName = f"{job_name_prefix}-{now}"
    input_path = f"gs://{event['bucket']}/{event['name']}"
    output_path = f"gs://seagen_dataflow/results_runner/dev-ihc-flex-output/{jobName}"
    config_file = 'gs://seagen_dataflow/ihc_config/config.json'
    
    config_bucket = 'seagen_dataflow'
    config_blob_name = 'ihc_config/config.json'
